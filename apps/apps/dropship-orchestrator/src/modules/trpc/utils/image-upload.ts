/**
 * Image download + multipart GraphQL upload utility for the Dropship Orchestrator.
 * Copied from Bulk Manager with CDN-compatible MIME detection (magic bytes).
 */
...
export async function downloadAndUploadImage(
  ...
  try {
    const response = await fetch(imageUrl, {
      headers: { "User-Agent": "SaleorDropshipOrchestrator/1.0" },
      redirect: "follow",
