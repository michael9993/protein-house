...
export class TransactionInitializeSessionUseCase {
  ...
  async execute(args: {
    ...
    // Record transaction to DynamoDB (non-blocking - don't fail webhook if recording fails)
    ...
    if (recordResult.isErr()) {
      // Log error but don't fail the webhook - transaction recording is for tracking purposes
      // The payment intent was successfully created, so we should return the client secret
      this.logger.warn("Failed to record transaction (non-blocking)", {
        error: recordResult.error,
        transaction: recordedTransaction,
      });
      // Continue execution - the payment intent was created successfully
    } else {
      this.logger.info("Wrote Transaction to storage", {
        transaction: recordedTransaction,
      });
    }
