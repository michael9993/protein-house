...
export class DynamodbAppConfigRepo implements AppConfigRepo {
  ...
  async getRootConfig(
    ...
    try {
      this.logger.debug("Querying DynamoDB for root config", {
        appId: access.appId,
        saleorApiUrl: access.saleorApiUrl.toString(),
        configsQueryPK: DynamoDbStripeConfig.accessPattern.getPK({
          appId: access.appId,
          saleorApiUrl: access.saleorApiUrl,
        }),
        mappingsQueryPK: DynamoDbChannelConfigMapping.accessPattern.getPK({
          appId: access.appId,
          saleorApiUrl: access.saleorApiUrl,
        }),
      });
      ...
      this.logger.debug("DynamoDB query results", {
        configsCount: configs.Items?.length ?? 0,
        mappingsCount: mappings.Items?.length ?? 0,
        configsItems: configs.Items?.map((item) => ({
          PK: item.PK,
          SK: item.SK,
        })),
        mappingsItems: mappings.Items?.map((item) => ({
          PK: item.PK,
          SK: item.SK,
        })),
      });
      ...
      this.logger.debug("Parsed configs and mappings", {
        parsedConfigsCount: parsedConfigs.length,
        parsedMappingsCount: parsedMappings.length,
        parsedConfigIds: parsedConfigs.map((c) => c.configId),
        parsedMappingChannels: parsedMappings.map((m) => m.channelId),
      });
