...
export class DynamoDBTransactionRecorderRepo implements TransactionRecorderRepo {
  ...
  async getTransactionByStripePaymentIntentId(
    ...
    // Note: DynamoDB doesn't support fallback appIds in the same way as PostgreSQL
    // The fallback logic would need to be implemented at a higher level
    // For now, we ignore fallbackAppIds for DynamoDB (legacy support)
    try {
