...
async function fetchAllMetadata(client: Client): Promise<MetadataEntry[]> {
...
async function mutateMetadata(client: Client, metadata: MetadataEntry[], appId: string) {
  ...
  return (
    mutationData?.updatePrivateMetadata?.item?.privateMetadata.map((md: { key: string; value: string }) => ({
      key: md.key,
      value: md.value,
    })) || []
  );
