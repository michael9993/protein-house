...
export class SmtpEmailSender implements ISMTPEmailSender {
  private TIMEOUT = 15000; // Increased to 15 seconds for large attachments
  ...
  async sendEmailWithSmtp({ smtpSettings, mailData }: SendMailArgs) {
    this.logger.debug("Sending an email with SMTP", {
      hasAttachments: mailData.attachments && mailData.attachments.length > 0,
      attachmentCount: mailData.attachments?.length || 0,
    });
