...
const ALL_TARGET_FIELDS = getTargetFields("customers");
...
export default function CustomersPage() {
  ...
  const importCustomers = trpcClient.customers.import.useMutation();
  ...
  const handleAutoMap = useCallback(
    async (headers: string[]) => {
      const result = await autoMapFields.mutateAsync({ headers, entityType: "customers" });
  ...
  const handleValidate = useCallback(
    async (rows: Record<string, string>[], mappings: Record<string, string>) => {
      return validateRows.mutateAsync({ rows, entityType: "customers", fieldMappings: mappings });
    },
    [validateRows]
  );
  ...
  const handleExecute = useCallback(
    async (
      ...
      const BATCH_SIZE = 20;
      const emailCol = Object.entries(mappings).find(([, t]) => t === "email")?.[0];
      ...
      for (let i = 0; i < rows.length; i += BATCH_SIZE) {
        ...
        const label = emailCol ? batch[0]?.[emailCol] : undefined;
        onProgress({
    ...
    [importCustomers, channelSlug, upsertMode]
  );

  const handleExport = useCallback(async (format: "csv" | "xlsx") => {
    const result = await trpcVanillaClient.customers.export.query({ format, first: 100 });
    if (format === "csv") downloadCSV(result.data, `customers-export-${Date.now()}.csv`);
    else downloadExcel(result.data, `customers-export-${Date.now()}.xlsx`);
  }, []);
    ...
    <AppLayout>
      <Box>
        ...
        <Text size={3} __color="#64748b" __display="block" marginBottom={6}>
          Import customer accounts and export customer data
        </Text>
        ...
        {tab === "import" && (
          <Box marginBottom={4}>
            ...
            <Box display="flex" gap={4} marginBottom={4} alignItems="flex-start" __flexWrap="wrap">
              <UpsertToggle checked={upsertMode} onChange={setUpsertMode} entityLabel="customers" matchDescription="match by email" />
        ...
        {tab === "export" && (
          <ExportDialog entityLabel="Customers" onExport={handleExport} />
        )}

        {tab === "delete" && (
          <BulkDeleteTab
            entityLabel="Customers"
            onDelete={(ids) => trpcVanillaClient.customers.bulkDelete.mutate({ ids })}
          />
        )}
        ...
        {tab === "template" && (
          <Box padding={6} borderRadius={4} __border="1px solid #e2e8f0">
            ...
            <Text size={2} __color="#64748b" __display="block" marginBottom={4}>
              ...
              Supports shipping/billing addresses, notes, language preferences, and metadata.
            </Text>
            <Box display="flex" gap={3}>
              <Button variant="secondary" onClick={() => generateTemplateCSV("customers", "Customers")}>Download CSV Template</Button>
              <Button variant="secondary" onClick={() => generateTemplateExcel("customers", "Customers")}>Download Excel Template</Button>
            </Box>
          </Box>
        )}
      </Box>
