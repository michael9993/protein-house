...
const attachAppToken = middleware(async ({ ctx, next }) => {
  ...
  if (!authData && ctx.saleorApiUrl.includes("trycloudflare.com")) {
    ...
    const baseLocalhost = "http://localhost:8000/graphql/";
    ...
    try {
      ...
      if (typeof (saleorApp.apl as any).getAll === 'function') {
        ...
        const tunnelAuth = allAuthData?.find((auth: any) => 
          auth.saleorApiUrl && auth.saleorApiUrl.includes("trycloudflare.com")
        );
