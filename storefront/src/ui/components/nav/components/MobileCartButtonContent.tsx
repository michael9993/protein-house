...
export function MobileCartButtonContent({ isActive, channel }: { isActive: boolean; channel: string }) {
  ...
  const className = "relative flex flex-col items-center justify-center gap-1 rounded-lg px-3 py-2 transition-all duration-200";
  ...
  const inner = (
    <>
      ...
      {lineCount > 0 && (
        <div
          className={clsx("absolute right-1 top-1 flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-bold text-white shadow-sm", lineCount > 9 && "h-5 w-6")}
