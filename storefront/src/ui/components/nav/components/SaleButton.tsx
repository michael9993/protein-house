...
export function SaleButton({ channel }: SaleButtonProps) {
  ...
  return (
    <LinkWithChannel
      ...
      className="nav-link-btn group relative flex items-center justify-center rounded-full px-3.5 py-1.5 text-sm font-bold tracking-wide transition-all duration-200"
      style={{
        color: branding.colors.error,
      }}
      onMouseEnter={(e) => {
        e.currentTarget.style.backgroundColor = `${branding.colors.error}0D`;
      }}
      onMouseLeave={(e) => {
