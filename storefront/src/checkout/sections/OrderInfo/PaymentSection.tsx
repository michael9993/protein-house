	...
	const getStatusConfig = () => {
		switch (paymentStatus) {
			case "authorized":
				return {
					icon: (
						<svg className="h-5 w-5" style={{ color: "var(--store-success)" }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
			...
			case "overpaid":
				return {
					icon: (
						<svg className="h-5 w-5" style={{ color: "var(--store-warning)" }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
	...
	return (
		<div className="px-6 py-4" data-testid="paymentStatus">
			<div className="flex items-start gap-3">
				<div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg" style={status.bgStyle}>
					{status.icon}
				</div>
				<div className="flex-1">
					...
					<p className="mt-0.5 text-sm" style={{ color: "var(--store-text-muted)" }}>{status.message}</p>
				</div>
			</div>
		</div>
	);
