	...
	return (
		<div className="space-y-3">
			...
			<button
				...
				aria-busy={isLoading}
				className="w-full rounded-md px-6 py-3.5 text-sm font-semibold text-white shadow transition-opacity disabled:cursor-not-allowed disabled:opacity-50 aria-disabled:cursor-not-allowed aria-disabled:opacity-50"
				style={{ backgroundColor: "var(--store-primary, #6366f1)" }}
			>
				{isLoading ? (
					<span className="flex items-center justify-center gap-2">
						<SmallSpinner />
						{t.processingOrderText ?? "Processing…"}
					</span>
				) : (
...
function SmallSpinner() {
	return (
		<svg
			...
			style={{ fill: "var(--store-primary-foreground, #fff)", color: "rgba(255,255,255,0.3)" }}
			viewBox="0 0 100 101"
			fill="none"
