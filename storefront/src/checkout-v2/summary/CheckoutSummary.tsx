...
export function CheckoutSummary({ channel: _channel }: CheckoutSummaryProps) {
	const { state, visibleLines } = useCheckoutState();
		...
		>
			...
			>
				...
				<span className="flex items-center gap-2">
					...
					<svg
						className={["h-4 w-4 text-neutral-400 transition-transform", isMobileExpanded ? "rotate-180" : ""].join(" ")}
						fill="none"
						viewBox="0 0 24 24"
						stroke="currentColor"
						strokeWidth={2}
			...
			<div
				...
				className={["px-4 py-4", "lg:block", isMobileExpanded ? "block" : "hidden"].join(" ")}
			>
