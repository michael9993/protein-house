...
docker exec -it saleor-api-dev pytest --reuse-db path/to/test.py -k name  # Specific test

# Linting & type checking
docker exec -it saleor-api-dev ruff check .
docker exec -it saleor-api-dev mypy saleor
...
- **Background jobs**: Celery (worker: `saleor-worker-dev`, scheduler: `saleor-scheduler-dev`)
- **Custom auth backend**: Supports JWT via `@saleor/auth-sdk`

After schema/model changes, restart dependent containers:
```bash
docker compose -f infra/docker-compose.dev.yml restart saleor-api-dev saleor-worker-dev saleor-scheduler-dev
```
...
2. **Rebuild schema**: `docker exec -it saleor-api-dev python manage.py build_schema`
3. **Regenerate frontend types**: `docker exec -it saleor-dashboard-dev pnpm generate` and `docker exec -it saleor-storefront-dev pnpm generate`
4. **Restart**: `docker compose -f infra/docker-compose.dev.yml restart saleor-api-dev`
...
- **Django shell**: `docker exec -it saleor-api-dev python manage.py shell` — test queries interactively
- **Check migrations**: `docker exec -it saleor-api-dev python manage.py showmigrations` — see applied/pending
- **Celery tasks**: Check worker logs: `docker compose -f infra/docker-compose.dev.yml logs saleor-worker-dev`
- **DB queries**: Connect via `docker exec -it saleor-postgres-dev psql -U saleor -d saleor`
