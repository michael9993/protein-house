...
def get_url_from_env(name, *, schemes=None) -> str | None:
    if name in os.environ:
        ...
        # If value is empty string, treat it as not set
...
# Allow tunnel URLs in development
# Django supports wildcards using leading dot (e.g., .trycloudflare.com matches all subdomains)
# When ALLOWED_HOSTS is set to '*', we allow all hosts (development only)
if DEBUG:
    if "*" in ALLOWED_HOSTS:
        # Allow all hosts in development - needed for dynamic tunnel URLs
        # This is safe because DEBUG=True and we're in development
        ALLOWED_HOSTS = ["*"]
    else:
        # Add common tunnel domain patterns to ALLOWED_HOSTS
        # Leading dot allows all subdomains (e.g., .trycloudflare.com matches *.trycloudflare.com)
        tunnel_domains = [
...
# CORS Configuration
# These settings are used by the ASGI CORS handler (saleor/asgi/cors_handler.py)
# to control Cross-Origin Resource Sharing for GraphQL API requests.
#
# ALLOWED_GRAPHQL_ORIGINS: List of allowed origins for GraphQL requests.
#   - Use "*" to allow all origins (development only, not recommended for production)
#   - Use specific origins like "http://localhost:3000" for production
#   - Supports wildcard patterns (e.g., "https://*.example.com")
#
# CORS_ALLOW_CREDENTIALS: Whether to allow credentials (cookies, auth headers) in CORS requests
# CORS_PREFLIGHT_MAX_AGE: Cache duration for preflight OPTIONS requests (in seconds)
# CORS_ALLOWED_HEADERS: Headers that are allowed in CORS requests
...
CORS_PREFLIGHT_MAX_AGE = int(os.environ.get("CORS_PREFLIGHT_MAX_AGE", 600))  # 10 minutes
CORS_ALLOWED_HEADERS = get_list(
    os.environ.get(
        "CORS_ALLOWED_HEADERS",
        "Origin, Content-Type, Accept, Authorization, Source-Service-Name, Authorization-Bearer",
    )
)
CORS_ALLOWED_METHODS = get_list(
    os.environ.get("CORS_ALLOWED_METHODS", "POST, OPTIONS")
)

# Development defaults: Allow common local development origins and tunnel domains
if DEBUG:
    if "*" in ALLOWED_GRAPHQL_ORIGINS:
        # If wildcard is set, it already allows all origins including tunnels
        pass
    else:
        # Add default local development origins
        _default_dev_origins = [
            "http://localhost:3000",  # Next.js storefront default
            "http://localhost:3001",  # Alternative storefront port
            "http://localhost:9000",  # Dashboard default
            "http://127.0.0.1:3000",
            "http://127.0.0.1:9000",
        ]
        # Merge with existing origins, avoiding duplicates
        for origin in _default_dev_origins:
            if origin not in ALLOWED_GRAPHQL_ORIGINS:
                ALLOWED_GRAPHQL_ORIGINS.append(origin)
        
        # Add tunnel domain patterns to allow all tunnel subdomains
        # fnmatch supports wildcards, so https://*.trycloudflare.com matches all subdomains
        tunnel_origin_patterns = [
            "https://*.trycloudflare.com",  # Cloudflared tunnels
            "http://*.trycloudflare.com",  # HTTP cloudflared tunnels
            "https://*.ngrok.io",  # Ngrok tunnels
            "http://*.ngrok.io",  # HTTP ngrok tunnels
            "https://*.ngrok-free.app",  # Ngrok free tier
            "http://*.ngrok-free.app",  # HTTP ngrok free tier
            "https://*.ngrok.app",  # Ngrok app domain
            "http://*.ngrok.app",  # HTTP ngrok app domain
        ]
        for pattern in tunnel_origin_patterns:
            if pattern not in ALLOWED_GRAPHQL_ORIGINS:
                ALLOWED_GRAPHQL_ORIGINS.append(pattern)
...
# Set GRAPHQL_QUERY_MAX_DEPTH=0 in env to disable (not recommended)
...
BUILTIN_PLUGINS = [
    ...
    # Removed: Adyen and Authorize.Net (Stripe-only checkout)
