...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Stripe payment gateway successfully installed and activated').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: The Stripe payment gateway app could not be installed, configured, and activated via the admin interface as required by the test plan.')
